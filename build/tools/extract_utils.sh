...
function setup_vendor() {
    ...
    export SM_ROOT="$3"
    if [ ! -d "$SM_ROOT" ]; then
        echo "\$SM_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$SM_ROOT/$OUTDIR" ]; then
        mkdir -p "$SM_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$SM_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDMK="$SM_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$SM_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$SM_ROOT"/vendor/sm/build/tools/smali/baksmali.jar
        export SMALIJAR="$SM_ROOT"/vendor/sm/build/tools/smali/smali.jar
    fi
    ...
    if grep "classes.dex" "$SM_TARGET" >/dev/null; then
...
function extract() {
    ...
    local SRC="$2"
    local OUTPUT_ROOT="$SM_ROOT"/"$OUTDIR"/proprietary
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$SM_ROOT"/vendor/sm/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$SM_ROOT"/"$OUTDIR"/radio
